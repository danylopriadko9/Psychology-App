...
export const ForgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const randomTokenExpiresAt = Date.now() + 1 * 60 * 15 * 1000; //15 min
...
export const ResetPassword = async (req: Request, res: Response) => {
  try {
    const { password, passwordRepeated, resetPasswordToken } = req.body;
    ...
    if (password !== passwordRepeated) {
      return res.status(400).json({
        success: false,
        message: "Password and repeated password need to be the same",
      });
    }
